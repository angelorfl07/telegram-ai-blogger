...
export async function createGeneratedPost(topic: string, mode: 'new' | 'rewrite' = 'new', useCache = true, imageStyle: string = 'Fotorrealista Premium'): Promise<GeneratedPost> {
  ...
  const article = await generateSeoArticle(topic, mode, imageStyle);
